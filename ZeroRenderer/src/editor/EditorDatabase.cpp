...
{
	if (!AssetPathExist(texturePath))
		return false;
	if (!AssetPathExist(matPath))
		return false;
	...
	if (textureSlot == TEX_SLOT_DIFFUSE_MAP)
		materialMeta.diffuseTextureGUID = texGUID;
	else if (textureSlot == TEX_SLOT_SPECULAR_MAP)
		materialMeta.specularTextureGUID = texGUID;
	MaterialMeta_SerializeTo(materialMeta, matPath);
	return true;
}
...
{
	string fromGUID;
	if (!TryGetGUIDFromAssetPath(fromPath, fromGUID))
		return;
	string toGUID;
	if (!TryGetGUIDFromAssetPath(toPath, toGUID))
		return;
}
...
void EditorDatabase::GenerateDefaultSceneMeta()
{
	...
	directLightGOMeta->transformMeta->position = vec3(-15.0f, 10.0f, -30.0f);
